...
// Place this function below your other admin sections (e.g., after AdminPosts) and before the styled-components block.
function AdminTicketsPanel() {
  const toast = useToast();
  const { user } = getAuth() || {};

  const [q, setQ] = useState("");
  const [stateFilter, setStateFilter] = useState("open"); // open | accepted | closed | all
  const [loading, setLoading] = useState(false);
  const [rows, setRows] = useState([]);
  ...
  // ---- styled (local to this panel; no inline styles) ----
  const TwoCol = useMemo(() => styled.div`
    display: grid;
  ...
  `, []);
  const TicketRow = useMemo(() => styled.tr`
    background: ${p => (p.$selected ? "#f1f5ff" : "transparent")};
  `, []);
  const RowLink = useMemo(() => styled.button`
    all: unset;
    cursor: pointer;
    color: #1f6feb;
    font-weight: 700;
  `, []);
  const Panel = useMemo(() => styled.div`
    display: grid;
    gap: 12px;
    padding: 10px;
  `, []);
  const Title = useMemo(() => styled.div`
    font-weight: 800;
    font-size: 16px;
  `, []);
  const Meta = useMemo(() => styled.div`
    color: #64748b;
    font-size: 13px;
  `, []);
  const ActionsWrap = useMemo(() => styled.div`
    display: flex;
    gap: 8px;
    flex-wrap: wrap;
  `, []);
  const MsgList = useMemo(() => styled.div`
    display: grid;
    gap: 8px;
  `, []);
  const MsgCard = useMemo(() => styled.div`
    border: 1px solid #e5e7eb;
    border-radius: 10px;
    padding: 8px;
    background: ${p => (p.$admin ? "#f8fafc" : "#ffffff")};
  `, []);
  const MsgMeta = useMemo(() => styled.div`
    font-size: 12px;
    color: #64748b;
    margin-bottom: 4px;
  `, []);
  const MsgBody = useMemo(() => styled.div`
    white-space: pre-wrap;
  `, []);
  const EmptyState = useMemo(() => styled.div`
    padding: 14px;
    color: #64748b;
  `, []);
  ...
  async function load() {
    ...
    try {
      ...
      setRows(d.tickets || []);
      if (selectedId && !(d.tickets || []).some(t => t.id === selectedId)) {
        setSelectedId(null);
        setMessages([]);
      }
    } catch (e) {
      toast.error(e?.message || "Nepavyko užkrauti bilietų");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function loadMessages(id) {
    ...
    setMsgsLoading(true);
    try {
      const d = await api(`/api/admin/tickets/${id}/messages`);
      setMessages(d.messages || []);
    } catch (e) {
      toast.error(e?.message || "Nepavyko užkrauti žinučių");
    } finally {
  ...
  // ---- actions ----
  async function patchTicket(id, body, okMsg = "Atnaujinta") {
    try {
      await api(`/api/admin/tickets/${id}`, { method: "PATCH", json: body });
      toast.success(okMsg);
      await load();
    } catch (e) {
      toast.error(e?.message || "Klaida atnaujinant");
  ...
  async function deleteTicket(id) {
    try {
      ...
      toast.success("Bilietas ištrintas");
      if (id === selectedId) {
        setSelectedId(null);
        setMessages([]);
      }
  ...
  function userLabel(t) {
    return t.user_name || t.username || t.user_email || `#${t.user_id}`;
  }
    ...
    <Section>
      ...
      <Flex>
        ...
        <SearchInput
          value={q}
          onChange={e => setQ(e.target.value)}
          placeholder="Paieška (pavadinimas, vartotojas, el. paštas)…"
        />
        <Ghost onClick={load}>{loading ? "Kraunama…" : "Ieškoti"}</Ghost>
      </Flex>
      ...
      <TwoCol>
        ...
        <TableCard>
          <Table>
            ...
            <tbody>
              {rows.map(t => (
                <TicketRow key={t.id} $selected={t.id === selectedId}>
                  <td>{t.id}</td>
                  <td>
                    <RowLink onClick={() => setSelectedId(t.id)}>
                      {t.title}
                    </RowLink>
                  </td>
                  <td>{userLabel(t)}</td>
                  <td>{t.state}</td>
                  <td>{t.last_message_at ? fmt(t.last_message_at) : "—"}</td>
                  <td style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                    {t.state !== "closed" ? (
                      <>
                        <Ghost onClick={() => patchTicket(t.id, { state: "accepted", assigned_admin_id: user?.id || null }, "Priimta")}>
                          Priimti
                        </Ghost>{" "}
                        <Primary onClick={() => patchTicket(t.id, { state: "closed" }, "Uždaryta")}>
                          Uždaryti
                        </Primary>{" "}
                      </>
                    ) : (
                      <Ghost onClick={() => patchTicket(t.id, { state: "open" }, "Atidaryta")}>Atidaryti</Ghost>
                    )}
                    <Small
                      onClick={() =>
                        patchTicket(
                          t.id,
                          { assigned_admin_id: t.assigned_admin_id ? null : (user?.id || null) },
                          t.assigned_admin_id ? "Atšaukta priskyrimą" : "Priskirta jums"
                        )
                      }
                    >
                      {t.assigned_admin_id ? "Unassign" : "Assign to me"}
                    </Small>{" "}
              ...
              {!rows.length && (
                <tr>
                  <td colSpan={6}>
                    <EmptyState>Nėra bilietų</EmptyState>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div>
          ...
          <TableCard>
            {!selected ? (
              <EmptyState>Pasirinkite bilietą kairėje.</EmptyState>
            ) : (
              <Panel>
                <div>
                  <Title>{selected.title}</Title>
                  <Meta>
                    #{selected.id} • {userLabel(selected)} • {selected.state} • sukurta {fmt(selected.created_at)}
                  </Meta>
                </div>
                ...
                <ActionsWrap>
                  {selected.state !== "closed" ? (
                    <>
                      <Ghost onClick={() => patchTicket(selected.id, { state: "accepted", assigned_admin_id: user?.id || null }, "Priimta")}>
                        Priimti
                      </Ghost>
                      <Primary onClick={() => patchTicket(selected.id, { state: "closed" }, "Uždaryta")}>
                        Uždaryti
                      </Primary>
                    </>
                  ) : (
                    <Ghost onClick={() => patchTicket(selected.id, { state: "open" }, "Atidaryta")}>Atidaryti</Ghost>
                  )}
                  <Small
                    onClick={() =>
                      patchTicket(
                        selected.id,
                        { assigned_admin_id: selected.assigned_admin_id ? null : (user?.id || null) },
                        selected.assigned_admin_id ? "Atšaukta priskyrimą" : "Priskirta jums"
                      )
                    }
                  >
                    {selected.assigned_admin_id ? "Unassign" : "Assign to me"}
                  </Small>
                  <Danger onClick={() => deleteTicket(selected.id)}>Trinti</Danger>
                </ActionsWrap>

                <Divider />

                <div>
                  <div style={{ fontWeight: 800, marginBottom: 6 }}>Žinutės</div>
                  {msgsLoading ? (
                    <EmptyState>Kraunama…</EmptyState>
                  ) : messages.length ? (
                    <MsgList>
                      {messages.map(m => (
                        <MsgCard key={m.id} $admin={m.sender_role === "admin"}>
                          <MsgMeta>
                            {m.sender_name || m.sender_role} • {fmt(m.created_at)}
                          </MsgMeta>
                    ...
                    <EmptyState>Žinučių nėra</EmptyState>
                  )}
                </div>
              </Panel>
            )}
