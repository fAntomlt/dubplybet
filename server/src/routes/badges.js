...
router.get("/users/:id/badges", async (req, res) => {
  ...
  const [rows] = await pool.query(
    `SELECT b.id,b.slug,b.name,b.description,b.color,b.bg,b.icon,ub.assigned_at
     ...
     WHERE ub.user_id = ?
     ORDER BY ub.assigned_at DESC`, [id]
  );
...
router.post("/admin/users/:userId/badges/:badgeId", requireAdmin, async (req, res) => {
...
router.delete("/admin/users/:userId/badges/:badgeId", requireAdmin, async (req, res) => {
  const { userId, badgeId } = req.params;
